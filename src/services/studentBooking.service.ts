  ...
  public async requestAppointment(
    ...
    try {
      ...
      await publishMessage(env.PUBSUB_NOTIFICATION_TOPIC, {
        ...
        content: `You have a new appointment request from ${studentDetails.email} for ${agenda} on ${proposedStart.toLocaleString()}. Please log in to your account to accept or decline the request.`,
        sendEmail: true,
        sendInApp: true,
      });
    } catch (error) {
    ...
    if (appointmentRequest.counselor_response === "accepted") {
      ...
      try {
        ...
        try {
          // Call pubsub notification for new appoinment request to notify the student
          await publishMessage(env.PUBSUB_NOTIFICATION_TOPIC, {
            userId: updatedRequest.counselor_id,
            type: "system_alerts",
            title: "Your Appointment Request Has Been Accepted",
            content: `Your appointment request for ${updatedRequest.agenda} on ${updatedRequest.proposed_start.toLocaleString()} has been accepted by the student. The appointment is now confirmed. Please log in to your account for more details.`,
            sendEmail: true,
            sendInApp: true,
          });
        } catch (error) {
  ...
  public async declineAppointmentRequest(
    ...
    try {
      ...
      await publishMessage(env.PUBSUB_NOTIFICATION_TOPIC, {
        ...
        content: `Your appointment request for ${updatedRequest.agenda} on ${updatedRequest.proposed_start.toLocaleString()} has been declined by the student.\n\nReason: ${updatedRequest.reason ?? "No reason provided"}\n\nPlease log in to your account for more details.`,
        sendEmail: true,
        sendInApp: true,
      });
    } catch (error) {
